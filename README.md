...
Persistent gain tracking records metadata only to `.agentgrep/tracking.jsonl` by default: sanitized command, cwd/project, input/output token estimates, saved tokens, savings percentage, and elapsed milliseconds. It never stores stdout or stderr. Set `AGENTGREP_TRACKING=0` to disable it, or `AGENTGREP_TRACKING_PATH=/path/to/tracking.jsonl` to move the ledger.
